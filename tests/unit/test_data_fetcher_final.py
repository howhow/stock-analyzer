...
class TestGetStockInfo:
    ...
    async def test_get_stock_info_fallback(self, fetcher):
        ...
        mock_source2.get_stock_info = AsyncMock(return_value=StockInfo(
            code="000001.SZ",
            name="平安银行",
            industry="银行",
            market_cap=1000000000.0,
...
class TestGetIntradayQuotes:
    ...
    async def test_get_intraday_quotes_not_available(self, fetcher):
        ...
        mock_source = AsyncMock()
        mock_source.name = "test_source"
        mock_source.get_intraday_quotes = AsyncMock(side_effect=Exception("Not available"))
...
class TestCircuitBreaker:
    ...
    async def test_get_circuit_breaker_status(self, fetcher):
        """测试获取熔断器状态"""
        mock_registry = Mock()
        mock_registry.get_all_status = Mock(return_value={
