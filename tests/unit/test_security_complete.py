...
class TestDataEncryptor:
    ...
    def test_encrypt_decrypt_with_string_key(self):
        """测试字符串密钥加密解密"""
        # 使用有效的 Fernet 密钥
        key = "Z7vN9xK2mP5qR8sT1uV4wY6zA3bC0dE_fGhIjKlMnOpQrStUvWxYz="
...
class TestGetApiKeyUser:
    ...
    async def test_get_api_key_user_valid(self):
        ...
        with patch("app.core.security.API_KEY_HEADER") as mock_header:
            mock_header.return_value = api_key
            
            result = await get_api_key_user(api_key, request)
    ...
    async def test_get_api_key_user_invalid_format(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_api_key_user(api_key, request)
    ...
    async def test_get_api_key_user_none(self):
        ...
        result = await get_api_key_user(None, request)
...
class TestGetCurrentUser:
    ...
    async def test_get_current_user_with_api_key(self):
        """测试获取当前用户 - API Key"""
        api_key_user = {"user_id": "api_user", "role": "service", "auth_type": "api_key"}
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--cov=app.core.security", "--cov-report=term-missing"])
