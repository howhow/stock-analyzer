...
class TestSystemAnalyzerFull:
    ...
    @pytest.mark.asyncio
    async def test_analyze_long_term(self, system_analyzer, stock_info, quotes, financial):
        """测试长期分析"""
    ...
    @pytest.mark.asyncio
    async def test_analyze_short_term(self, system_analyzer, stock_info, quotes, financial):
        """测试短期分析"""
        result = await system_analyzer.analyze(
            stock_info, 
            quotes, 
    ...
    @pytest.mark.asyncio
    async def test_analyze_with_bullish_trend(self, system_analyzer, stock_info, financial):
        """测试牛市趋势分析"""
        # 创建上升趋势数据
        quotes = []
        for i in range(30):
    ...
    @pytest.mark.asyncio
    async def test_analyze_with_bearish_trend(self, system_analyzer, stock_info, financial):
        """测试熊市趋势分析"""
        # 创建下降趋势数据
        quotes = []
        for i in range(30):
    ...
    @pytest.mark.asyncio
    async def test_analyze_integration(self, system_analyzer, stock_info, quotes, financial):
        """测试分析师和交易员集成"""
        result = await system_analyzer.analyze(
            stock_info, 
            quotes, 
