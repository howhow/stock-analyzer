...
def run_cmd(cmd: List[str], cwd: Optional[str] = None) -> Tuple[int, str, str]:
    ...
    result = subprocess.run(cmd, capture_output=True, text=True, cwd=cwd or os.getcwd())
...
def check_makefile_coverage(
    new_files: List[str], new_dirs: List[str], covered: Set[str]
) -> Tuple[bool, List[str]]:
    """
    ...
    for f in new_files:
        ...
        # 根目录文件默认被覆盖（lint/test 覆盖整个项目）
        if not dir_path:
            is_covered = True
        # 检查文件本身是否在 covered 中
        elif os.path.basename(f) in covered:
            is_covered = True
        else:
            # 检查上级目录
            for c in covered:
...
def check_large_files(
    files: List[str], max_lines: int = MAX_LINES
